...
class NextQATrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            sys_prompt = '''You are given video, a question, and five options of answers to the question which are indexed by A, B, C, D, E. Your task is to select the correct answer index from the five options of answers to the question according to the video.'''
    ...
    def _vqa_question(self, idx, is_example=False):
        ...
        question_prompt = f'''<question> {question} </question> <options>: A.{answer_choices[0]} B.{answer_choices[1]} C.{answer_choices[2]} D.{answer_choices[3]} E.{answer_choices[4]} </options> <answer> {correct_answer_in_example}'''
        if is_example:
