...
class RunnerBase:
    ...
    def resume_ckpt_path(self):
        return self.config.run_cfg.get("resume_ckpt_path", False)
    
    @property
    def ckpt_path(self):
        return self.config.run_cfg.get("ckpt_path", "")
    ...
    @property
    def metrics(self):
        return self.config.run_cfg.get("evaluate_metrics", None)
            ...
            if len(self.valid_splits) > 0:
                for split_name in self.valid_splits:
                    ...
                    val_log = self.eval_epoch(
                        split_name=split_name, metrics=self.metrics, cur_epoch=cur_epoch
                    )
    ...
    def train(self):
        ...
        # resume from checkpoint if specified
        if not self.evaluate_only and self.ckpt_path != "":
    ...
    @torch.no_grad()
    def eval_epoch(self, split_name, cur_epoch, metrics, skip_reload=False):
        """
        Evaluate the model on a given split.
        ...
        for k in after_keys:
            if torch.equal(state_dict_orig[k], state_dict_after[k]):
                print(k)
        ###
    ...
    def _save_checkpoint(self, cur_epoch, is_best=False):
        ...
        save_to = os.path.join(
            # self.output_dir,
            self.config.run_cfg.output_dir, 
    ...
    def log_stats(self, stats, split_name):
        if isinstance(stats, dict):
            ...
            with open(os.path.join("/mnt/disks/chfuab/video-llama/output", "log.txt"), "a") as f:
