...
class VideoLLAMA(Blip2Base):
    ...
    def __init__(
        ...
        self.audio_encoder.load_state_dict(torch.load("{}/imagebind_huge.pth".format(imagebind_ckpt_path), map_location=torch.device("cuda:2"), mmap=True, weights_only=False))
    ...
    def encode_videoaudioQformer(self, 
        ...
        # finding position_id and calculate position embeddings
            ...
            # calculate audio position embedding
                position_id_list_A = torch.tensor(position_id_list_A)
            ...
            # calculate visual position embedding:
                position_id_img_frms = torch.tensor([pair[0] for pair in image_frame_idx_all[k]])
